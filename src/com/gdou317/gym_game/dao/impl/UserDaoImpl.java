package com.gdou317.gym_game.dao.impl;

import com.gdou317.gym_game.dao.UserDao;
import com.gdou317.gym_game.model.User;
...
public class UserDaoImpl extends HibernateDaoSupport implements UserDao
{
    ...
    public User getByName(String name)
    {
        ...
        //将所有的数据查询出来并放到List集合里 User是表对应的实例名称
        List users = session.createQuery("from gamemes where user_name='" + name + "'").list();
    ...
    {
        Session session = sessionFactory.getCurrentSession();
        List users = session.createQuery("from gamemes where user_no='" + no + "'").list();
        return (User)users.get(0);
    }
    ...
    {
        Session session = sessionFactory.getCurrentSession();
        String sql = "delete from gamemes where id=:id";
    ...
    {
        Session session = sessionFactory.getCurrentSession();
        String sql = "delete from gamemes where gameID=:no";
        Query query = session.createQuery(sql);
